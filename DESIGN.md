...
Join-link tokens are separate from machine callback auth.
...
Require a per-session machine token generated by the control plane, stored as `sessions.machine_token_hash`, and injected into cloud-init as plaintext only on the session droplet.
...
```txt
POST /api/sessions
  -> insert session(status=created, title, slug, machine_token_hash, droplet_region, droplet_size, image_id, timestamps)
...
- Per-session machine token for droplet-to-control callbacks stored on `sessions`.
